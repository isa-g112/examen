...
<form action="{{ route('companies.store') }}" method="POST">
  ...
  <div class="mb-3">
    ...
    <input name="legal_representative_name" class="form-control" value="{{ old('legal_representative_name') }}">
  </div>
  <div class="mb-3">
    <label class="form-label">Legal Representative Lastname</label>
    <input name="legal_representative_lastname" class="form-control" value="{{ old('legal_representative_lastname') }}">
  </div>
  <div class="mb-3">
    <label class="form-label">Legal Representative Email</label>
    <input name="legal_representative_email" class="form-control" value="{{ old('legal_representative_email') }}">
  </div>
  <div class="mb-3">
    ...
    <select name="users_iduser" class="form-control">
      ...
      @foreach($users as $user)
        <option value="{{ $user->iduser }}" @selected(old('users_iduser') == $user->iduser)>{{ $user->name }} ({{ $user->email }})</option>
      @endforeach
    </select>
  </div>
